...
;
RunMotors:
    ; uchar *vp = &vels[run_vix][0];
    ; uchar *dp = &dirs[run_vix][0];
    ; uchar *p  = &pos[0];
    banksel vels
    lfsr	FSR0,vels
    lfsr	FSR1,dirs
    ...
    ; FSR2 is not indexed by run_vix, so no need to adjust it

    ; At this point:
    ;     FSR0 points to vels[run_vix] 8bit
    ;     FSR1 points to dirs[run_vix] 8bit
    ;     FSR2 points to pos[c]        16bit
    ;
    
  
    ;
    ; Now we loop through channels ABCD (0 - 3) doing the following:
    ;
...
rm_chan_loop:		; for loop
    ...
    ; if ( G_freq == 0 ) { pos[c]  = vels[run_vix][c]; }
    ; else               { pos[c] += vels[run_vix][c]; }
    ;
...
; if ( pos[chan] >= MAXFREQ ) { Step(chan, dir, 0); pos[chan] -= MAXFREQ; } // step
; else                        { Step(chan, dir, 1); pos[chan] -= 0;      }  // unstep
;
    ; if (pos[chan] >= MAXFREQ)
    ; 16bit compare GTE
    ;                         ;   Cycles
...
rm_maxfreq_gte:                   ;  |      |
    ; Step(0, dir, 0);            ;  |      |
    movff  rm_chan,fstep_arg_chan ;  2      |   ; rm_chan -> fstep_arg_chan
    movff  INDF1,fstep_arg_dir    ;  2      |   ; dir[run_vix][0] -> fstep_arg_dir  (NOTE: INDF1 doesn't increment)
    movlw  0			  ;  1      |   ; UN-STEP
    movwf  fstep_arg_val          ;  1      |
...
rm_maxfreq_done:
    ...
    ; [2]   if ( got_vels ) {           // new vels successfully received from IBMPC?
    ;         // Swap new/run arrays
    ;         G_freq    = zr;
    ;         run_vix  ^= one;          // start using IBMPC data for running motors
    ;         new_vix  ^= one;          // old run vels become new buffer for IBMPC
    ;         got_vels  = zr;           // reset got_vels flag
...
rm_G_freq_gt:                 ;   |     |_________
    ; [2] if ( got_vels ) {       |_______        |
    ;    // new vels from IBMPC?  |       |       |
    ;                         ; GVSKIP GVNOSKIP   |
    ;                         ; ------ --------   |
    btfsc got_vels,0	      ;   3       1       |  ; bit test skip if clr
...
rm_got_vels_if:               ;   |       |       |
    ...
    ; // Swap new/run arrays
    ; G_freq    = 0;
    ; run_vix  ^= 1; // start using IBMPC data for running motors
    ; new_vix  ^= 1; // old run vels become new buffer for IBMPC
    ; got_vels  = 0; // reset got_vels flag
    ; IBMPC_IRQ = 1; // trigger IBMPC IRQ just this iter
