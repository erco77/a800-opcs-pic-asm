...
rv_velptr       res 1           ; ptr to vels[new_vix]
rv_dirptr       res 1           ; ptr to dirs[new_vix]
...
;   vels[run_vix][chan], dirs[run_vix][chan]
;
run_vix		res 1	    ; 0|1 index for vels[ix][chan] and dirs[ix][chan]
new_vix		res 1	    ; index for new vels from IBMPC (opposite of run_vix's value)
		
; Flag to indicate when PC has finished sending us new vels
; so next IRQ can swap run_vix/new_vix and start using new vels.
got_vels	res 1	    ; flag indicating if IBMPC finished sending us vels
...
MAIN_PROG CODE                          ; let linker place main program
    ...
    #include "a800-run-motors.asm"      ; defines RunMotors(): handles integer math to generate steps
...
START:
    ...
    ; run_vix = 0;  \_ must always be
    ; new_vix = 1;  /  opposite values
    movlw   0x00
    movwf   run_vix
    movlw   0x01
    movwf   new_vix
...
main_loop:
    ...
    ; PORTB |= 0b01010101; // force step bits only, dir bits untouched
    movf    LATB,W
    iorlw   b'01010101'
    movwf   PORTB
    ...
    movff   G_portb, PORTB
    goto    main_loop	; COMMENT OUT TO HAVE A VELS CHANGE EACH IRQ
